...
def set_sample_config(config_path: Path) -> Dict[str, Any]:
    ...
    config_to_test: Dict[str, Any] = {
        "client_id": os.getenv('TAP_STRAVA_CLIENT_ID') or file_config_params.get('client_id', None),
        "client_secret": os.getenv('TAP_STRAVA_CLIENT_SECRET') or file_config_params.get('client_secret', None),
        "refresh_token": os.getenv('TAP_STRAVA_REFRESH_TOKEN') or file_config_params.get('refresh_token', None)
    }
...
def get_samples_from_streams() -> Dict[str, Any]:
    ...
    cli_out = subprocess.run(['tap-strava','--config', 'ENV', '--test'], stdout=subprocess.PIPE).stdout.decode('utf-8')
